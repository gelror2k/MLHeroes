...
| Third-party API | Rone Arena (`https://arena.rone.dev/api`, docs at `/api/docs`) | Unofficial public MLBB ranked-stats API, HTTPS, no key. Midterm requirement: the app must consume one external API |
...
- **Third-party ranked stats (2026-09-25).** The only win/pick/ban rates in the app come live from the Rone Arena API through `services/metaService.ts` → `hooks/use-meta.ts` → `components/live-meta.tsx` (`LiveMetaCard` on Home, `HeroMetaCard` on the profile). They are never cached or written to our DB. Heroes are looked up by name (the API ignores case, spaces and symbols; all 133 roster names matched on 2026-09-25); a 404 means "not listed" and renders an empty note, not an error. The cards load independently of our API so a failure in one never hides the other. The predecessor `mlbb-stats.ridwaanhall.com` no longer resolves — don't switch back to it.
